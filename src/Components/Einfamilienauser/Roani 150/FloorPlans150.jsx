...
export default function FloorPlans150() {
  const floors = [
  ...
  return (
    <section className="fp150">
      <div className="fp150__inner">
        {/* Tabs */}
        <div className="fp150__tabs" role="tablist" aria-label="Floor selector">
          {floors.map((f, i) => (
            <button
              key={f.key}
              role="tab"
              aria-selected={active === i}
              className={`fp150__tab ${active === i ? "is-active" : ""}`}
              onClick={() => setActive(i)}
            >
              {f.label}
            </button>
        ...
        {/* Foto e planit */}
        <div className="fp150__stage">
          <img
            className="fp150__img"

...
export default function FloorPlans() {
  ...
  return (
    <section className="fp140">
      <div className="fp140__inner">
        {/* Tabs */}
        <div className="fp140__tabs" role="tablist" aria-label="Floor selector">
          {floors.map((f, i) => (
            <button
              key={f.key}
              role="tab"
              aria-selected={active === i}
              className={`fp140__tab ${active === i ? "is-active" : ""}`}
              onClick={() => setActive(i)}
            >
              {f.label}
            </button>
        ...
        {/* Foto e planit */}
        <div className="fp140__stage">
          <img
            className="fp140__img"

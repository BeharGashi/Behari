...
export default function MarigonaFloorPlans() {
  const floors = [
  ...
  return (
    <section className="mgfp">
      <div className="mgfp__inner">
        {/* Tabs */}
        <div className="mgfp__tabs" role="tablist" aria-label="Floor selector">
          {floors.map((f, i) => (
            <button
              key={f.key}
              role="tab"
              aria-selected={active === i}
              className={`mgfp__tab ${active === i ? "is-active" : ""}`}
              onClick={() => setActive(i)}
            >
              {f.label}
            </button>
